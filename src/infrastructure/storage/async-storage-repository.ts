...
const KURMUSIC_PREFIX = '@kurmusic:';

class AsyncStorageRepository implements StorageRepository {
	private prefixKey(key: string): string {
		return `${KURMUSIC_PREFIX}${key}`;
	}

	private unprefixKey(key: string): string {
		return key.startsWith(KURMUSIC_PREFIX) ? key.slice(KURMUSIC_PREFIX.length) : key;
	}
	...
	async getKeys(prefix?: string): Promise<Result<string[], Error>> {
		try {
			...
			let kurmusicKeys = allKeys
				.filter((key) => key.startsWith(KURMUSIC_PREFIX))
				.map((key) => this.unprefixKey(key));

			if (prefix !== undefined) {
				kurmusicKeys = kurmusicKeys.filter((key) => key.startsWith(prefix));
			}

			return ok(kurmusicKeys);
		} catch (error) {
			return err(
	...
	async clear(): Promise<Result<void, Error>> {
		try {
			const allKeys = await AsyncStorage.getAllKeys();
			const kurmusicKeys = allKeys.filter((key) => key.startsWith(KURMUSIC_PREFIX));

			if (kurmusicKeys.length > 0) {
				await AsyncStorage.multiRemove(kurmusicKeys);
			}
